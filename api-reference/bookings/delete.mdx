...
```bash cURL
curl -X DELETE "https://your-site.com/wp-json/latepoint-api/v1/bookings/123?force=true" \
  -H "X-API-Key: lp_live_1234567890abcdef"
```
...
<Note>
  The current implementation checks for `confirmed` and `completed` statuses to require force deletion.
</Note>
