...
<ParamField body="email" type="string">
  Customer's email (optional for API, must be unique if provided)
...
const response = await fetch('https://your-site.com/wp-json/latepoint-api/v1/customers', {
  ...
  body: JSON.stringify(customerWithoutEmail)
});

const result = await response.json();
console.log('Customer created without email:', result.data);
```
...
response = requests.post(
    ...
    data=json.dumps(customer_without_email)
)

result = response.json()
print('Customer created without email:', result['data'])
```
...
curl_setopt_array($curl, [
    ...
    CURLOPT_POSTFIELDS => json_encode($customer_without_email)
]);
